-- BECA Assessment Platform - Complete Database Fix
-- Run this to fix all table issues

-- ============================================================
-- 1. CREATE assessment_results TABLE (if missing)
-- ============================================================
...
-- ============================================================
-- 2. CREATE attempt_answers TABLE (if missing)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_answers_result ON attempt_answers(result_id);
...
-- ============================================================
-- 4. FIX: If attempt_answers exists but missing result_id, drop and recreate
-- ============================================================
-- This only runs if the table exists but is broken
...
CREATE INDEX IF NOT EXISTS idx_modules_assessment ON assessment_modules(assessment_id);
...
CREATE INDEX IF NOT EXISTS idx_questions_module ON assessment_questions(module_id);
CREATE INDEX IF NOT EXISTS idx_questions_type ON assessment_questions(question_type);
...
CREATE INDEX IF NOT EXISTS idx_assignments_trainee ON assessment_assignments(trainee_id);
...
CREATE INDEX IF NOT EXISTS idx_files_question ON assessment_question_files(question_id);
...
-- Results policies
CREATE POLICY IF NOT EXISTS "Users can view own results" ON assessment_results
  FOR SELECT USING (auth.uid() = user_id OR
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND user_role IN ('admin', 'superadmin', 'trainer')));

CREATE POLICY IF NOT EXISTS "Admins can insert results" ON assessment_results
  FOR INSERT WITH CHECK (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND user_role IN ('admin', 'superadmin', 'trainer'))
  );

CREATE POLICY IF NOT EXISTS "Admins can update results" ON assessment_results
  FOR UPDATE USING (
    EXISTS (SELECT 1 FROM profiles WHERE id = auth.uid() AND user_role IN ('admin', 'superadmin', 'trainer'))
  );

-- Answers policies
CREATE POLICY IF NOT EXISTS "Users can view own answers" ON attempt_answers
  FOR SELECT USING (
    EXISTS (
...
CREATE POLICY IF NOT EXISTS "Users can insert own answers" ON attempt_answers
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM assessment_results ar 
      WHERE ar.id = attempt_answers.result_id 
